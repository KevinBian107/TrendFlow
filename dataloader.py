...
def get_dataloader(args, dataset, split='train'):
    """
    Creates a DataLoader using stratified sampling via WeightedRandomSampler.
    This will oversample minority classes so that, on average, each batch is balanced.
    """

    scenario_labels = [ex.scenario_label for ex in dataset[split]]

    label_counts = Counter(scenario_labels)

    weights = [1.0 / label_counts[lbl] for lbl in scenario_labels]
    ...
    if split == 'train':
        ...
        sampler = SequentialSampler(range(len(scenario_labels)))

    dataloader = DataLoader(
        dataset[split],
        sampler=sampler,
        batch_size=args.batch_size,
        collate_fn=dataset[split].collate_func
    )
